...
async def cashout(interaction: discord.Interaction):
    ...
    old_time: datetime = user.last_cashout.get_time()  # 1st call
    new_time: datetime = datetime.now().replace(microsecond=0)
    user.last_cashout.set(new_time)  # 2nd call

    diff = (new_time - old_time).total_seconds()

    current_balance = user.balance.get_float() + diff * 2  # 3rd call
    user.balance.set(current_balance)  # 4th call
